...
## 🛠 Running the Queue (Background Worker)

The compiled binary supports multiple execution modes. By default, it starts the API server. To run the background workers from the same binary, simply add the `--worker` flag:
...
### 2. Monitoring & Scale
In production, you should run at least one instance of the server and one instance of the worker as separate processes:
```bash
