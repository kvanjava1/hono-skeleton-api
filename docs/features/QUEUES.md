...
const startWorkers = () => {
    ...
    // 2. Register workers
    // Argument 1 must match the name in createQueue('...')
...
### Production
The worker should be run as its own service. If using a process manager like PM2:
```bash
pm2 start "bun run src/worker.ts" --name "my-app-worker"
```
