import { findByName, getUsageCount } from '../../repositories/apiClient.repository.ts';
...
const checkClient = async () => {
    ...
    if (args.length < 1) {
        console.log('\n❌ Usage: bun run src/utils/m2m/check.ts "<name>"');
        console.log('Example: bun run src/utils/m2m/check.ts "nexus"\n');
        process.exit(1);
    }
