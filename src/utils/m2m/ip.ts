 ...
 * Usage: 
 * bun run src/utils/m2m/ip.ts add <clientId> <ip>
 * bun run src/utils/m2m/ip.ts remove <clientId> <ip>
...
const manageIp = async () => {
    ...
    if (args.length < 2) {
        console.log('\n❌ Usage:');
        console.log('  bun run src/utils/m2m/ip.ts add <clientId> <ip>');
        console.log('  bun run src/utils/m2m/ip.ts remove <clientId> <ip>');
        console.log('  bun run src/utils/m2m/ip.ts clear <clientId>');
        process.exit(1);
    }
