 ...
 * Usage: 
 * bun run src/utils/m2m/status.ts "<clientId>" "<active|suspended>"
 */
...
const updateStatus = async () => {
    ...
    if (args.length < 2) {
        console.log('\n❌ Usage: bun run src/utils/m2m/status.ts "<clientId>" "<active|suspended>"');
        console.log('Example: bun run src/utils/m2m/status.ts "web_01" "suspended"\n');
        process.exit(1);
    }
