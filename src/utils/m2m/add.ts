 ...
 * Usage: 
 * bun run src/utils/m2m/add.ts "Client Name" "client_id" "client_secret" [rate_limit] [allowed_ips]
 */
...
const addClient = async () => {
    ...
    if (args.length < 3) {
        console.log('\n❌ Usage: bun run src/utils/m2m/add.ts <name> <clientId> <clientSecret> [rateLimit] [allowedIps]');
        console.log('Example: bun run src/utils/m2m/add.ts "My Website" web_01 secret_123 5000 "127.0.0.1, 192.168.1.1"');
        process.exit(1);
    }
